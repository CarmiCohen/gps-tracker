# Device-Specific Adaptations (v8.9.34)
...
- **Logic**: Transitions to "Far" are ignored unless sustained for the full window.

### 1.3. GPS Polling Stabilization (Samsung A15)
The system enforces a 1000ms polling interval (`A15_STABLE_GPS_POLLING_MS`) to prevent the GPS hardware from entering aggressive power-save modes.
...
- **Action**: Triggers a "Revival Pulse" (GPS refresh + WakeLock renewal).
