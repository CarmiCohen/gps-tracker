# Documentation Organization & Forensic Audit Standard (v9.3.6)
...
- **STATUS/SOT_MASTER_REQUIREMENTS.md**: The definitive operational spec.
...
- **STATUS/RELEASE_HISTORY.md**: Chronological version history (The Changelog).
- **STATUS/RESOLUTION_ARCHIVE.md**: Technical resolution trail for bugs, architectural deviations, and "FIXED" tasks.
- **STATUS/VERIFICATION_MANIFEST.md**: The **Verification & Compliance Manifest**. Formal proof of implementation and historical record.
- **STATUS/QA_VALIDATION_STATUS.md**: The backlog of pending unit tests, integration tests, and manual field validations.
...
1.  **History**: Log the version milestone in `STATUS/RELEASE_HISTORY.md`.
2.  **Issues**: Move completed tasks to the "FIXED" section in `STATUS/VERIFICATION_MANIFEST.md`. (Issue #312)
3.  **SoT**: Update the core constants and requirements in `STATUS/SOT_MASTER_REQUIREMENTS.md`.
...
1.  Locate the version milestone in **STATUS/RELEASE_HISTORY.md**.
2.  Cross-reference the resolved items in **STATUS/VERIFICATION_MANIFEST.md**.
3.  Execute the **Verification Manifest** in **STATUS/VERIFICATION_MANIFEST.md** and the **Backlog** in **STATUS/QA_VALIDATION_STATUS.md** against the current source code to ensure architectural standards are maintained.

## 4. Forensic Unification (v9.3.6)
As of v9.3.6, the forensic model is simplified and hardened. Forensic traceability is maintained by injecting the build version at emission points and is enhanced by **Ghost Mode UX** staleness indicators (R338), acknowledged SIT synchronization (Issue #194), and **Log Spatial Anchoring** (Issue #208).
