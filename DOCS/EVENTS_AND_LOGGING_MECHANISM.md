# Events & Logging Mechanism (v8.9.10)
...
- **Log Spatial Anchor (v8.9.10)**: All events are automatically anchored with `lat`/`lng` coordinates to enable historical map reconstruction.
...
6.  **Ghost Mode UX (Issue 193)**: Visual staleness indicators are applied to log entries when telemetry is older than 10s.
...
Legacy version tags have been removed from data models. Traceability is maintained by injecting the build version at the emission layer and is enhanced by **acknowledged SIT synchronization**, **Power Forensic Parity**, and **Spatial Anchoring** (v8.9.10).
