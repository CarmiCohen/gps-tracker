...
2.  **Accuracy Gate (Issue #303)**: Ignores fixes with accuracy > `TRAJECTORY_REJECTION_ACCURACY_MULT` (3.0f) the recent high-accuracy average (`HIGH_ACCURACY_THRESHOLD_METERS` 35.0f). (Formerly #433)
...
4.  **Behavioral Sentinel (Issue #302)**: Validates GPS movement against accelerometer vibration data. Uses `VIBRATION_STATIONARY_THRESHOLD` (0.12f) to distinguish movement. (Formerly #432)
...
- **JUMP**: Validated GPS spike rejected by sensor cross-validation. Uses `JUMP_GATE_VISUAL_JITTER_METERS` (10.0m) as a minimum rejection floor (Issue #304 - Formerly #434).
- **OUTLIER**: Extreme coordinate jump (> `OUTLIER_DISTANCE_THRESHOLD` 2000m) rejected by physics check.
- **PROMOTED**: Trajectory confirmed through consistent movement within `TRAJECTORY_PROMOTION_WINDOW_MS` (30s), overriding initial jitter filters (Issue #285 - Formerly #415).
