...
1.  **Monotonic Guard**: Rejects updates with backward-moving hardware timestamps (`CLOCK_REGRESSION_GATE_MS` 100ms). (Issue #311 - Formerly #283-A)
...
- **Data Parity (Issue #337 - Formerly #192)**: Historical points include raw speed, bearing, and battery current (`currentMa`).
- **Ghost Mode (Issue #338 - Formerly #193)**: Visual staleness indicators applied to markers and accuracy circles when telemetry > `TELEMETRY_UI_STALE_THRESHOLD_MS` (10s).
...
- **PROMOTED**: Trajectory confirmed through consistent movement within `TRAJECTORY_PROMOTION_WINDOW_MS` (30s), overriding initial jitter filters (Issue #367 - Formerly #285).
