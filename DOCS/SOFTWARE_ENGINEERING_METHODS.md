# Modern Software Engineering Methods & Tools (v8.9.37)
...
- **ViewModel**: `MainViewModel.kt` centralizes business logic and telemetry processing. Decoupled into domain-specific UseCases (Issue #115).
- **Repository Pattern**: `MainRepository.kt` abstracts data storage.
- **Modularization (Issue #58/#110)**: Physical isolation of the Tracking Engine into `:core:engine` (pure `java-library`), ensuring zero dependency leakage from the Android framework into the core logic (Issue #100).
...
- **SDK 35**: Fully aligned with Android 15 edge-to-edge, Room migrations (v39), and foreground service requirements (Issue #45).

## 7. Forensic Unification
As of v8.9.37, the forensic model is simplified and hardened. Legacy version tags have been removed from data models. Traceability is maintained at the emission layer, further enhanced by **Power Forensic Parity** (Issue #192) and **Log Spatial Anchoring** (Issue #208).
