# Video Streaming and CameraX Integration Documentation (v8.9.10)
...
### Current State (v8.9.10)
- **Permissions**: `CAMERA` and `RECORD_AUDIO` are not requested.
- **Service Types**: `TrackerService` requests `microphone` only for the Acoustic Sentinel, not for streaming.
- **UI**: All video icons and containers remain purged.
- **Dependencies**: CameraX and WebRTC libraries are excluded from the build.

---
*Note: Video streaming is not part of the v8.9.10 "Hardened Engine" specification. Legacy versioning fields have been purged from the system.*
