...
*   **DEVICE STATE**: The current operational state of the tracker logic.
    ...
    *   `COOLING`: The device has exceeded `MAX_SAFE_TEMPERATURE_CELSIUS` (46.0°C) and is in power-save mode. (Issue #352)
    *   `STALLED`: GPS hardware is active but providing frozen coordinates (> 60s `GPS_STALL_THRESHOLD_MS`). (Issue #198)
        *   **Escalated Revival**: System attempts soft-revival every 120s (`GPS_REVIVAL_RETRY_INTERVAL_MS`). After 3 failures, it escalates to a CRITICAL hardware lock alert. (Issue #341)
...
*   **[PROMOTED]**: A trajectory confirmed through 30s (`TRAJECTORY_PROMOTION_WINDOW_MS`) of consistent movement, bypassing jitter filters. (Issue #367)
...
*   **Battery Drain (currentMa)**: Real-time battery current in mA. (Issue #337)
