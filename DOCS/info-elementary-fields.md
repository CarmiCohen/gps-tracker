...
*   **DEVICE STATE**: The current operational state of the tracker logic.
    ...
    *   `COOLING`: The device has exceeded `MAX_SAFE_TEMPERATURE_CELSIUS` (46.0°C) and is in power-save mode. (Issue #273)
    *   `STALLED`: GPS hardware is active but providing frozen coordinates (> 60s `GPS_STALL_THRESHOLD_MS`). (Issue #198)
        *   **Escalated Revival**: System attempts soft-revival every 120s (`GPS_REVIVAL_RETRY_INTERVAL_MS`). After 3 failures, it escalates to a CRITICAL hardware lock alert. (Issue #124)
...
*   **[PROMOTED]**: A trajectory confirmed through 30s (`TRAJECTORY_PROMOTION_WINDOW_MS`) of consistent movement, bypassing jitter filters. (Issue #285)
*   **[BATT HEALTH]**: Triggered by a **Steep Discharge** event (> 5% drop in 10 minutes). (Issue #221)
...
*   **Uptime**: Process lifespan (`uptimeMs`). (Issue #271)
...
*   **Tr Max**: Worst accuracy recorded in the session (High-water mark). (Issue #214)
*   **Age Index**: Staleness of the current position fix (10s `GPS_UI_FAIL_THRESHOLD_MS` gray-out/Ghost Mode). (Issue #193)
...
*   **Bayesian Expansion**: The UI displays spatial uncertainty growing at `PENDING_UNCERTAINTY_GROWTH_RATE_MPS` (15m/s) during blackouts if `locationPendingReason` is active. (Issue #221)
...
*   **Proximity**: Detection of device covering/handling. Hardened for Samsung A15 (Issue #191).

## 5. SIT (Sitting) Detection
These fields are primarily visualized in the **Analytical Ribbons** overlay and track mechanical "sitting" events. (Issue #282)
...
*   **Battery Drain (currentMa)**: Real-time battery current in mA. (Issue #192)
...
*   **BAT Ribbon**: Indicates binary **Steep Discharge** status.
...
*   **Lux Baseline**: Environmental light EMA (dual-rate Slow/Fast). (Issue #266)
...
*   **Muzzle Window**: 2000ms suppression window (`MUZZLE_WINDOW_DURATION_MS`) to eliminate false tamper triggers during sync. (Issue #191)
