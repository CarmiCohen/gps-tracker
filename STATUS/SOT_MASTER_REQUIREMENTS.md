...
*   **Startup Transition Authority (R658)**: The Main thread MUST remain silent during activity transitions and initial composition. Any non-critical initialization or IPC requests MUST be deferred until after the first frame is rendered and the activity reaches the `RESUMED` state to prevent Davey stalls >500ms. (Issue #658, July.30.46)
*   **JNI Initialization Integrity (R659)**: The `MbrainHardwareManager` MUST verify the integrity of the native library state before every JNI call. If a failure is detected, the system MUST attempt a background re-initialization without blocking the caller or leaking native resources. (Issue #659, July.30.46)
