...
*   **SOT ID 349**: Signaling Pipeline Hardening - Decoupled `ConnectivitySuite` from Android `ConnectivityManager` and direct HTTP calls using `NetworkProvider` and `SignalingTransport` interfaces. Enforced HTTP 2xx check for keep-alive probes to prevent premature failure counter resets during server-side errors. (Resolved Sep.16.10)
