# System Source of Truth (SoT) - July.20.00
...
*   **Startup Maintenance Authority (R104)**: To prevent I/O bottlenecks and ANRs during cold starts or schema migrations on low-end hardware (Samsung A15), the system MUST execute a proactive `deepPruneLogs` operation on `Dispatchers.IO` immediately upon initialization. This operation MUST prioritize the shedding of routine heartbeats (`watchdog_stats`, `pulse`) while strictly preserving forensic integrity (`isImportant` or `isSpecial` logs). (July.20.00 / Issue #104)
