# SOT Master Requirements & Hardening Status (Sep.19.02)
...
*   **SOT ID 361**: Redundant Battery Baseline Capture Remediation - Guarded the `init` coroutine loop in `HardwareSuite.kt` with `isStarted.get()`. This prevents `ForensicAuditor.captureRevivalStart()` from being triggered during app initialization or idle states, ensuring battery baselines are only captured when tracking is actively engaged (R-ID 361). (Resolved Sep.19.02)
...
*   **Chapter 31.25 (Baseline Capture Gating)**: PASSED - Verified that battery baseline capture is strictly gated by suite active state (Sep.19.02)
...
---
*Next Audit: Sep.20.00. (Sep.19.02)*
