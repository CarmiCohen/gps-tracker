...
*   **Step Detector Permission Authority (R107)**: To ensure hardware Step Detector availability on API 29+, the system MUST explicitly declare, request, and track the `android.permission.ACTIVITY_RECOGNITION` permission. Failures in registration MUST be surfaced in the Diagnostics UI. (July.20.07 / Issue #107)
...
*   **Cold-Start Hardening Authority (R955b)**: To prevent Main-thread frame skipping and ANRs on low-end hardware, the system MUST implement a mandatory 500ms staggered delay before starting base observations. (July.19.01 / Issue #099)
...
*   **Differential IPC Polling Authority (R951)**: To prevent main thread starvation on low-end hardware, the system MUST use differential polling for permissions. (v9.3.45 / Issue #095)
...
*   **Automatic Mode Transition Delay (R925)**: The app MUST pause for two seconds on the landing page before automatically entering Tracker or Viewer mode. (Issue #092 / v9.3.36)
...
*   **Binary Telemetry Authority (R988)**: The system MUST prioritize binary Protobuf-based telemetry for high-frequency tracker updates. (v9.3.25)
*   **Alias-Aware Identity Uniqueness (R182b)**: Tracker and Viewer IDs MUST NOT conflict with reserved legacy aliases. (v9.3.25)
*   **Samsung A15 Hardening Authority (R405)**: The system MUST prioritize background persistence on Samsung A15 devices. (v9.3.25)
*   **Relay Configuration Authority (R404)**: The system MUST enforce a centralized relay configuration. (v9.3.18)
