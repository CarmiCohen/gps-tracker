# System Source of Truth (SoT) - v8.9.50
...
*   **Standardized Alert Titles**: Authority resides in `EngineConstants.kt` (e.g., "This device: Internet Lost", "Tracker: Tamper Detected"). (Issue #392)
...
*   **Authoritative Spatial Anchoring (R325)**: **`maxAccuracy` is the exclusive authority** for Geofence transitions, Distance Violations, and Trajectory Deduplication (Persistence & Parking Breakout). (Issue #325 / Issue #423)
...
*   **Bayesian Uncertainty Growth (Issue #328)**: Uncertainty expands at **15.0m/s** (Moving) and **1.5m/s** (Stationary) when location fixes are pending, with a safety cap of **33.3m/s**. (Issue #383)
...
*   **GtoEngine Optimization**: Authorization for Towing (10m/s) and Work (5m/s) speed thresholds. (Issue #264/Issue #404)
...
*   **Xiaomi System Ready**: Hardened gating using `is_xiaomi_manual_override` and autostart verification. Includes **15s suppression** and **60s recovery** gates for heuristic revival. (Issue #190/Issue #406)
*   **Samsung A15**: Proximity debounce (5000ms) and post-sync hysteresis (500ms).
*   **Samsung S21 FE**: Explicit background activity required for 10Hz GPS polling. (Issue #401)
...
*   **Siren Engine**: Authority for **30s** auto-stop (Issue #429) and **15s** resume cooldown. (Issue #410)
*   **Timing Authority**: Startup log muzzle **10s**, Heartbeat interval **1h**, Marker pruning threshold **50**. (Issue #409)
