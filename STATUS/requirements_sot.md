...
| `ACOUSTIC_FLOOR_MIN_DB` | 25.0dB | Absolute floor for environmental calibration. (Issue #292) |
...
| `JUMP_GATE_VISUAL_JITTER_METERS`| 10.0m | Tier 3 Visual Jitter distance floor. (Issue #304) |
...
| `LANDING_PAGE_PAUSE_MS` | 2,000ms | Mandatory landing page pause for auto-mode entry (R925). |
...
| `TRAJECTORY_REJECTION_ACCURACY_MULT`| 3.0x | Multiplier for trajectory-based outlier rejection. (Issue #303) |
...
### 4.2. Session Lifecycle (R921, R925, R926)
*   **Exhaustive State Reset (R921)**: Mandatory clearing of all telemetry, stats, and session state during role transitions in `MainViewModel`.
*   **Landing Page Pause (R925)**: Mandatory `LANDING_PAGE_PAUSE_MS` (2,000ms) pause on landing page before auto-entry to ensure stability.
*   **Auto-Start Resilience (R926)**: Guaranteed background service launch during auto-transition to prevent stale/gray data.

### 4.3. LED Signaling Logic (R922, R942, R943)
*   **Tracker Role**: LEDs (INT, SRV, VWR, GPS) reflect local hardware and connectivity status for immediate feedback. Does not require a pulse from the Viewer.
*   **Viewer Role**: LEDs are gated by the age of the incoming telemetry pulse from the Tracker. If the Tracker is offline or the telemetry is stale, the remote indicators (TRK, DAT, GPS) will reflect this state (RED).
*   **Dynamic Peer Labeling (R942)**: The primary peer activity badge is labeled **VWR** (Viewer) in Tracker mode and **TRK** (Tracker) in Viewer mode to clearly identify the remote counterpart.
*   **Redundancy Suppression (R943)**: The **DAT** (Data Pipeline) badge is hidden in Tracker mode, as local pipeline health is already represented by INT and SRV indicators.
*   **Badge Mapping**:
    *   **INT**: Local Internet Connectivity.
    *   **SRV**: Connection to the Relay Server.
    *   **TRK/VWR**: Peer Activity (Viewer pulse for Tracker / Tracker pulse for Viewer).
    *   **DAT**: Data pipeline health (Sync/Telemetry stream) - *Viewer Only*.
    *   **GPS**: GPS Fix Status (Local hardware for Tracker / Remote state for Viewer).
