# Architectural Simplification Ideas (vAug.25.02)
...
5.  **Standardized LRU Provider**: (New) Evaluate replacing the custom `ShadowCache` wrapper with `androidx.collection.LruCache` to reduce custom concurrency boilerplate, while maintaining the `ReentrantLock` for 100Hz burst safety.
