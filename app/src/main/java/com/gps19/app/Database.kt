...
data class LogEntity(
    ...
    @ColumnInfo(defaultValue = "0.0") val lat: Double = 0.0,
    @ColumnInfo(defaultValue = "0.0") val lng: Double = 0.0,
...
data class HistoryEntity(
    ...
    @ColumnInfo(defaultValue = "0.0") val gpsIndex: Double = 0.0,
    @ColumnInfo(defaultValue = "0.0") val noiseIdx: Double = 0.0,
    @ColumnInfo(defaultValue = "0.0") val luxIdx: Double = 0.0,
    @ColumnInfo(defaultValue = "0.0") val vibeIdx: Double = 0.0,
    @ColumnInfo(defaultValue = "1.0") val proxIdx: Double = 1.0, 
    @ColumnInfo(defaultValue = "0.0") val liftIdx: Double = 0.0,
    @ColumnInfo(defaultValue = "0.0") val snrIdx: Double = 0.0,
    @ColumnInfo(defaultValue = "0.0") val tiltIdx: Double = 0.0,
    @ColumnInfo(defaultValue = "0.0") val baroIdx: Double = 0.0,
    @ColumnInfo(defaultValue = "0.0") val verticalVelocity: Double = 0.0,
    @ColumnInfo(defaultValue = "0.0") val sitVz: Double = 0.0, 
    @ColumnInfo(defaultValue = "0") val sitVzTs: Long = 0L,
    @ColumnInfo(defaultValue = "0.0") val sitDz: Double = 0.0,
    ...
    @ColumnInfo(defaultValue = "0.0") val sitBaro: Double = 0.0,
    @ColumnInfo(defaultValue = "0.0") val sitTilt: Double = 0.0,
    @ColumnInfo(defaultValue = "0.0") val sitShock: Double = 0.0,
    ...
    companion object {
        val MIGRATION_54_55 = object : Migration(54, 55) {
            override fun migrate(db: SupportSQLiteDatabase) {
                // Issue #096: Harmonize logs table
                ...
                val selectL = "id, localId, timestamp, message, type, isImportant, deviceId, viewerId, count, extremeValue, durationMs, isSpecial, specialColor, " +
                    ...
                    (if (colsL.contains("lat")) "lat" else "0.0") + ", " +
                    (if (colsL.contains("lng")) "lng" else "0.0") + ", " +
                    (if (colsL.contains("accuracy")) "accuracy" else "0.0") + ", " +
                    (if (colsL.contains("maxAccuracy")) "maxAccuracy" else "0.0") + ", " +
                ...
                // Issue #096: Harmonize trail_points table
                db.execSQL("CREATE TABLE trail_points_new (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, lat REAL NOT NULL, lng REAL NOT NULL, timestamp INTEGER NOT NULL, isViewerTrail INTEGER NOT NULL, isJump INTEGER NOT NULL, isHindsightCorrected INTEGER NOT NULL DEFAULT 0, accuracy REAL NOT NULL DEFAULT 0.0, maxAccuracy REAL NOT NULL DEFAULT 0.0)")
                
                val cursorT = db.query("PRAGMA table_info(trail_points)")
                val colsT = mutableSetOf<String>(); while(cursorT.moveToNext()) colsT.add(cursorT.getString(1)); cursorT.close()
                
                val selectT = "id, lat, lng, timestamp, isViewerTrail, isJump, " +
                    (if (colsT.contains("isHindsightCorrected")) "isHindsightCorrected" else "0") + ", " +
                    (if (colsT.contains("accuracy")) "accuracy" else "0.0") + ", " +
                    (if (colsT.contains("maxAccuracy")) "maxAccuracy" else "0.0")

                db.execSQL("INSERT INTO trail_points_new SELECT $selectT FROM trail_points")
                ...
                // Issue #096: Harmonize violations table
                db.execSQL("CREATE TABLE violations_new (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, lat REAL NOT NULL, lng REAL NOT NULL, type TEXT NOT NULL, ts INTEGER NOT NULL, accuracy REAL NOT NULL DEFAULT 0.0, maxAccuracy REAL NOT NULL DEFAULT 0.0)")
                
                val cursorV = db.query("PRAGMA table_info(violations)")
                val colsV = mutableSetOf<String>(); while(cursorV.moveToNext()) colsV.add(cursorV.getString(1)); cursorV.close()
                
                val selectV = "id, lat, lng, type, ts, " +
                    (if (colsV.contains("accuracy")) "accuracy" else "0.0") + ", " +
                    (if (colsV.contains("maxAccuracy")) "maxAccuracy" else "0.0")

                db.execSQL("INSERT INTO violations_new SELECT $selectV FROM violations")
                ...
                // Issue #096: Harmonize connection_history table
                db.execSQL("CREATE TABLE connection_history_new (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, ts INTEGER NOT NULL, rtt INTEGER NOT NULL, isConnected INTEGER NOT NULL, isGap INTEGER NOT NULL, hasGps INTEGER NOT NULL, isTick INTEGER NOT NULL, ribbonKey TEXT NOT NULL, gpsIndex REAL NOT NULL DEFAULT 0.0, noiseIdx REAL NOT NULL DEFAULT 0.0, luxIdx REAL NOT NULL DEFAULT 0.0, vibeIdx REAL NOT NULL DEFAULT 0.0, proxIdx REAL NOT NULL DEFAULT 1.0, liftIdx REAL NOT NULL DEFAULT 0.0, snrIdx REAL NOT NULL DEFAULT 0.0, tiltIdx REAL NOT NULL DEFAULT 0.0, baroIdx REAL NOT NULL DEFAULT 0.0, verticalVelocity REAL NOT NULL DEFAULT 0.0, sitVz REAL NOT NULL DEFAULT 0.0, sitVzTs INTEGER NOT NULL DEFAULT 0, sitDz REAL NOT NULL DEFAULT 0.0, isBatterySteepDischarge INTEGER NOT NULL DEFAULT 0, remoteSig INTEGER NOT NULL DEFAULT 10, isCoolingModeActive INTEGER NOT NULL DEFAULT 0, speed REAL NOT NULL DEFAULT 0.0, bearing REAL NOT NULL DEFAULT 0.0, isSitDetected INTEGER NOT NULL DEFAULT 0, isSitActive INTEGER NOT NULL DEFAULT 0, sitBaro REAL NOT NULL DEFAULT 0.0, sitTilt REAL NOT NULL DEFAULT 0.0, sitShock REAL NOT NULL DEFAULT 0.0, currentMa INTEGER NOT NULL DEFAULT 0, locationPendingReason TEXT NOT NULL DEFAULT 'NONE', accuracy REAL NOT NULL DEFAULT 0.0, maxAccuracy REAL NOT NULL DEFAULT 0.0, isAnchorLocked INTEGER NOT NULL DEFAULT 0)")
                ...
                val selectH = "id, ts, rtt, isConnected, isGap, hasGps, isTick, ribbonKey, " +
                    (if (colsH.contains("gpsIndex")) "gpsIndex" else "0.0") + ", " +
                    (if (colsH.contains("noiseIdx")) "noiseIdx" else "0.0") + ", " +
                    (if (colsH.contains("luxIdx")) "luxIdx" else "0.0") + ", " +
                    (if (colsH.contains("vibeIdx")) "vibeIdx" else "0.0") + ", " +
                    (if (colsH.contains("proxIdx")) "proxIdx" else "1.0") + ", " +
                    (if (colsH.contains("liftIdx")) "liftIdx" else "0.0") + ", " +
                    (if (colsH.contains("snrIdx")) "snrIdx" else "0.0") + ", " +
                    (if (colsH.contains("tiltIdx")) "tiltIdx" else "0.0") + ", " +
                    (if (colsH.contains("baroIdx")) "baroIdx" else "0.0") + ", " +
                    (if (colsH.contains("verticalVelocity")) "verticalVelocity" else "0.0") + ", " +
                    (if (colsH.contains("sitVz")) "sitVz" else "0.0") + ", " +
                    (if (colsH.contains("sitVzTs")) "sitVzTs" else "0") + ", " +
                    (if (colsH.contains("sitDz")) "sitDz" else "0.0") + ", " +
                    ...
                    (if (colsH.contains("speed")) "speed" else "0.0") + ", " +
                    (if (colsH.contains("bearing")) "bearing" else "0.0") + ", " +
                    (if (colsH.contains("isSitDetected")) "isSitDetected" else "0") + ", " +
                    (if (colsH.contains("isSitActive")) "isSitActive" else "0") + ", " +
                    (if (colsH.contains("sitBaro")) "sitBaro" else "0.0") + ", " +
                    (if (colsH.contains("sitTilt")) "sitTilt" else "0.0") + ", " +
                    (if (colsH.contains("sitShock")) "sitShock" else "0.0") + ", " +
                    (if (colsH.contains("currentMa")) "currentMa" else "0") + ", " +
                    (if (colsH.contains("locationPendingReason")) "locationPendingReason" else "'NONE'") + ", " +
                    (if (colsH.contains("accuracy")) "accuracy" else "0.0") + ", " +
                    (if (colsH.contains("maxAccuracy")) "maxAccuracy" else "0.0") + ", " +
                    (if (colsH.contains("isAnchorLocked")) "isAnchorLocked" else "0")

                db.execSQL("INSERT INTO connection_history_new SELECT $selectH FROM connection_history")
                ...
                // Issue #096: Harmonize pending_status_updates table
                db.execSQL("CREATE TABLE pending_status_updates_new (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, lat REAL NOT NULL, lng REAL NOT NULL, speed REAL NOT NULL, accuracy REAL NOT NULL, bearing REAL NOT NULL, battery INTEGER NOT NULL, temp REAL NOT NULL, isCharging INTEGER NOT NULL, currentMa INTEGER NOT NULL DEFAULT 0, timestamp INTEGER NOT NULL, gpsTs INTEGER NOT NULL DEFAULT 0, satsView INTEGER NOT NULL, satsUsed INTEGER NOT NULL, name TEXT, maxAccuracy REAL NOT NULL, distToTracker REAL, distToHome REAL, snrIdx REAL NOT NULL DEFAULT 0.0, tiltIdx REAL NOT NULL DEFAULT 0.0, baroIdx REAL NOT NULL DEFAULT 0.0, isBatterySteepDischarge INTEGER NOT NULL DEFAULT 0, isCoolingModeActive INTEGER NOT NULL DEFAULT 0, isSitDetected INTEGER NOT NULL DEFAULT 0, isSitActive INTEGER NOT NULL DEFAULT 0, sitVz REAL NOT NULL DEFAULT 0.0, sitVzTs INTEGER NOT NULL DEFAULT 0, sitDz REAL NOT NULL DEFAULT 0.0, verticalVelocity REAL NOT NULL DEFAULT 0.0, sitBaro REAL NOT NULL DEFAULT 0.0, sitTilt REAL NOT NULL DEFAULT 0.0, sitShock REAL NOT NULL DEFAULT 0.0, isStorageLow INTEGER NOT NULL DEFAULT 0, isStorageCritical INTEGER NOT NULL DEFAULT 0, isPowerSaveMode INTEGER NOT NULL DEFAULT 0, standbyBucket INTEGER NOT NULL DEFAULT -1, netInterface TEXT NOT NULL DEFAULT 'UNKNOWN', lastValidFixRealtime INTEGER NOT NULL DEFAULT 0, locationPendingReason TEXT NOT NULL DEFAULT 'NONE', isAnchorLocked INTEGER NOT NULL DEFAULT 0, trackerState TEXT NOT NULL DEFAULT 'UNKNOWN')")
                ...
                val selectP = "id, lat, lng, speed, accuracy, bearing, battery, temp, isCharging, " +
                    ...
                    (if (colsP.contains("gpsTs")) "gpsTs" else "0") + ", satsView, satsUsed, name, maxAccuracy, distToTracker, distToHome, " +
                    (if (colsP.contains("snrIdx")) "snrIdx" else "0.0") + ", " +
                    (if (colsP.contains("tiltIdx")) "tiltIdx" else "0.0") + ", " +
                    (if (colsP.contains("baroIdx")) "baroIdx" else "0.0") + ", " +
                    ...
                    (if (colsP.contains("sitVz")) "sitVz" else "0.0") + ", " +
                    (if (colsP.contains("sitVzTs")) "sitVzTs" else "0") + ", " +
                    (if (colsP.contains("sitDz")) "sitDz" else "0.0") + ", " +
                    (if (colsP.contains("verticalVelocity")) "verticalVelocity" else "0.0") + ", " +
                    (if (colsP.contains("sitBaro")) "sitBaro" else "0.0") + ", " +
                    (if (colsP.contains("sitTilt")) "sitTilt" else "0.0") + ", " +
                    (if (colsP.contains("sitShock")) "sitShock" else "0.0") + ", " +
                    ...
                    (if (colsP.contains("lastValidFixRealtime")) "lastValidFixRealtime" else "0") + ", " +
                    (if (colsP.contains("locationPendingReason")) "locationPendingReason" else "'NONE'") + ", " +
                    (if (colsP.contains("isAnchorLocked")) "isAnchorLocked" else "0") + ", " +
                    (if (colsP.contains("trackerState")) "trackerState" else "'UNKNOWN'")

                db.execSQL("INSERT INTO pending_status_updates_new SELECT $selectP FROM pending_status_updates")
        ...
        val MIGRATION_52_53 = object : Migration(52, 53) {
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                val selectH = "id, ts, rtt, isConnected, isGap, hasGps, isTick, ribbonKey, " +
                    ...
                    (if (colsH.contains("vibeIdx")) "vibeIdx" else "0.0") + ", " +
                    (if (colsH.contains("proxIdx")) "proxIdx" else "1.0") + ", " +
                    (if (colsH.contains("warmIdx")) "warmIdx" else "0.0") + ", " +
                    (if (colsH.contains("snrIdx")) "snrIdx" else "0.0") + ", " +
                    (if (colsH.contains("tiltIdx")) "tiltIdx" else "0.0") + ", " +
                    (if (colsH.contains("baroIdx")) "baroIdx" else "0.0") + ", " +
                    (if (colsH.contains("verticalVelocity")) "verticalVelocity" else "0.0") + ", " +
                ...
                // Issue #043: Harmonize pending_status_updates with explicit DEFAULTS
                db.execSQL("CREATE TABLE pending_status_updates_new (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, lat REAL NOT NULL, lng REAL NOT NULL, speed REAL NOT NULL, accuracy REAL NOT NULL, bearing REAL NOT NULL, battery INTEGER NOT NULL, temp REAL NOT NULL, isCharging INTEGER NOT NULL, currentMa INTEGER NOT NULL DEFAULT 0, timestamp INTEGER NOT NULL, gpsTs INTEGER NOT NULL DEFAULT 0, satsView INTEGER NOT NULL, satsUsed INTEGER NOT NULL, name TEXT, maxAccuracy REAL NOT NULL, distToTracker REAL, distToHome REAL, snrIdx REAL NOT NULL DEFAULT 0.0, tiltIdx REAL NOT NULL DEFAULT 0.0, baroIdx REAL NOT NULL DEFAULT 0.0, isBatterySteepDischarge INTEGER NOT NULL DEFAULT 0, isCoolingModeActive INTEGER NOT NULL DEFAULT 0, isSitDetected INTEGER NOT NULL DEFAULT 0, isSitActive INTEGER NOT NULL DEFAULT 0, sitVz REAL NOT NULL DEFAULT 0.0, sitVzTs INTEGER NOT NULL DEFAULT 0, sitDz REAL NOT NULL DEFAULT 0.0, verticalVelocity REAL NOT NULL DEFAULT 0.0, sitBaro REAL NOT NULL DEFAULT 0.0, sitTilt REAL NOT NULL DEFAULT 0.0, sitShock REAL NOT NULL DEFAULT 0.0, isStorageLow INTEGER NOT NULL DEFAULT 0, isStorageCritical INTEGER NOT NULL DEFAULT 0, isPowerSaveMode INTEGER NOT NULL DEFAULT 0, standbyBucket INTEGER NOT NULL DEFAULT -1, netInterface TEXT NOT NULL DEFAULT 'UNKNOWN', lastValidFixRealtime INTEGER NOT NULL DEFAULT 0, locationPendingReason TEXT NOT NULL DEFAULT 'NONE', isAnchorLocked INTEGER NOT NULL DEFAULT 0)")
        ...
        val MIGRATION_51_52 = object : Migration(51, 52) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE connection_history ADD COLUMN isAnchorLocked INTEGER NOT NULL DEFAULT 0")
                db.execSQL("ALTER TABLE pending_status_updates ADD COLUMN isAnchorLocked INTEGER NOT NULL DEFAULT 0")
            }
        }
        val MIGRATION_50_51 = object : Migration(50, 51) {
            override fun migrate(db: SupportSQLiteDatabase) {
                val cursor = db.query("PRAGMA table_info(pending_status_updates)")
                var exists = false
                ...
                cursor.close()
                if (!exists) {
                    db.execSQL("ALTER TABLE pending_status_updates ADD COLUMN name TEXT")
                }
            }
        }
        val MIGRATION_49_50 = object : Migration(49, 50) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE trail_points ADD COLUMN accuracy REAL NOT NULL DEFAULT 0.0")
                db.execSQL("ALTER TABLE trail_points ADD COLUMN maxAccuracy REAL NOT NULL DEFAULT 0.0")
            }
        }
        val MIGRATION_48_49 = object : Migration(48, 49) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE connection_history ADD COLUMN accuracy REAL NOT NULL DEFAULT 0.0")
                db.execSQL("ALTER TABLE connection_history ADD COLUMN maxAccuracy REAL NOT NULL DEFAULT 0.0")
            }
        }
        val MIGRATION_47_48 = object : Migration(47, 48) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE violations ADD COLUMN accuracy REAL NOT NULL DEFAULT 0.0")
                db.execSQL("ALTER TABLE violations ADD COLUMN maxAccuracy REAL NOT NULL DEFAULT 0.0")
            }
        }
        val MIGRATION_46_47 = object : Migration(46, 47) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE logs ADD COLUMN maxAccuracy REAL NOT NULL DEFAULT 0.0")
            }
        }
        ...
        val MIGRATION_45_44 = object : Migration(45, 44) { override fun migrate(db: SupportSQLiteDatabase) {} } // placeholder
        ...
        val MIGRATION_43_44 = object : Migration(43, 44) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE connection_history ADD COLUMN tiltIdx REAL NOT NULL DEFAULT 0.0")
                db.execSQL("ALTER TABLE connection_history ADD COLUMN baroIdx REAL NOT NULL DEFAULT 0.0")
                db.execSQL("ALTER TABLE pending_status_updates ADD COLUMN tiltIdx REAL NOT NULL DEFAULT 0.0")
                db.execSQL("ALTER TABLE pending_status_updates ADD COLUMN baroIdx REAL NOT NULL DEFAULT 0.0")
            }
        }
        ...
        val MIGRATION_39_40 = object : Migration(39, 40) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE logs ADD COLUMN accuracy REAL NOT NULL DEFAULT 0.0")
            }
        }
        val MIGRATION_38_39 = object : Migration(38, 39) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE logs ADD COLUMN lat REAL NOT NULL DEFAULT 0.0")
                db.execSQL("ALTER TABLE logs ADD COLUMN lng REAL NOT NULL DEFAULT 0.0")
            }
        }
        ...
        val MIGRATION_32_33 = object : Migration(32, 33) {
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                db.execSQL("CREATE TABLE connection_history_new (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, ts INTEGER NOT NULL, rtt INTEGER NOT NULL, isConnected INTEGER NOT NULL, isGap INTEGER NOT NULL, hasGps INTEGER NOT NULL, isTick INTEGER NOT NULL, ribbonKey TEXT NOT NULL, gpsIndex REAL NOT NULL DEFAULT 0.0, noiseIdx REAL NOT NULL DEFAULT 0.0, luxIdx REAL NOT NULL DEFAULT 0.0, vibeIdx REAL NOT NULL DEFAULT 0.0, proxIdx REAL NOT NULL DEFAULT 0.0, liftIdx REAL NOT NULL DEFAULT 0.0, snrIdx REAL NOT NULL DEFAULT 0.0, verticalVelocity REAL NOT NULL DEFAULT 0.0, sitVz REAL NOT NULL DEFAULT 0.0, sitDz REAL NOT NULL DEFAULT 0.0, isBatterySteepDischarge INTEGER NOT NULL DEFAULT 0, remoteSig INTEGER NOT NULL DEFAULT 10, isCoolingModeActive INTEGER NOT NULL DEFAULT 0, speed REAL NOT NULL DEFAULT 0.0, bearing REAL NOT NULL DEFAULT 0.0, isSitDetected INTEGER NOT NULL DEFAULT 0, isSitActive INTEGER NOT NULL DEFAULT 0, sitBaro REAL NOT NULL DEFAULT 0.0, sitTilt REAL NOT NULL DEFAULT 0.0, sitShock REAL NOT NULL DEFAULT 0.0)")
                ...
                db.execSQL("CREATE TABLE pending_status_updates_new (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, lat REAL NOT NULL, lng REAL NOT NULL, speed REAL NOT NULL, accuracy REAL NOT NULL, bearing REAL NOT NULL, battery INTEGER NOT NULL, temp REAL NOT NULL, isCharging INTEGER NOT NULL, currentMa INTEGER NOT NULL DEFAULT 0, timestamp INTEGER NOT NULL, gpsTs INTEGER NOT NULL DEFAULT 0, satsView INTEGER NOT NULL, satsUsed INTEGER NOT NULL, name TEXT, maxAccuracy REAL NOT NULL, distToTracker REAL, distToHome REAL, snrIdx REAL NOT NULL DEFAULT 0.0, isBatterySteepDischarge INTEGER NOT NULL DEFAULT 0, isCoolingModeActive INTEGER NOT NULL DEFAULT 0, isSitDetected INTEGER NOT NULL DEFAULT 0, isSitActive INTEGER NOT NULL DEFAULT 0, sitVz REAL NOT NULL, sitVzTs INTEGER NOT NULL DEFAULT 0, sitDz REAL NOT NULL, verticalVelocity REAL NOT NULL DEFAULT 0.0, sitBaro REAL NOT NULL, sitTilt REAL NOT NULL DEFAULT 0.0, sitShock REAL NOT NULL, isStorageLow INTEGER NOT NULL DEFAULT 0, isStorageCritical INTEGER NOT NULL DEFAULT 0, isPowerSaveMode INTEGER NOT NULL DEFAULT 0, standbyBucket INTEGER NOT NULL DEFAULT -1, netInterface TEXT NOT NULL DEFAULT 'UNKNOWN')")
        ...
        val MIGRATION_35_36 = object : Migration(35, 36) {
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                db.execSQL("CREATE TABLE pending_status_updates_v36 (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, lat REAL NOT NULL, lng REAL NOT NULL, speed REAL NOT NULL, accuracy REAL NOT NULL, bearing REAL NOT NULL, battery INTEGER NOT NULL, temp REAL NOT NULL, isCharging INTEGER NOT NULL, currentMa INTEGER NOT NULL DEFAULT 0, timestamp INTEGER NOT NULL, gpsTs INTEGER NOT NULL DEFAULT 0, satsView INTEGER NOT NULL, satsUsed INTEGER NOT NULL, name TEXT, maxAccuracy REAL NOT NULL, distToTracker REAL, distToHome REAL, snrIdx REAL NOT NULL DEFAULT 0.0, isBatterySteepDischarge INTEGER NOT NULL DEFAULT 0, isCoolingModeActive INTEGER NOT NULL DEFAULT 0, isSitDetected INTEGER NOT NULL DEFAULT 0, isSitActive INTEGER NOT NULL DEFAULT 0, sitVz REAL NOT NULL DEFAULT 0.0, sitDz REAL NOT NULL DEFAULT 0.0, verticalVelocity REAL NOT NULL DEFAULT 0.0, sitBaro REAL NOT NULL DEFAULT 0.0, sitTilt REAL NOT NULL DEFAULT 0.0, sitShock REAL NOT NULL DEFAULT 0.0, isStorageLow INTEGER NOT NULL DEFAULT 0, isStorageCritical INTEGER NOT NULL DEFAULT 0, isPowerSaveMode INTEGER NOT NULL DEFAULT 0, standbyBucket INTEGER NOT NULL DEFAULT -1, netInterface TEXT NOT NULL DEFAULT 'UNKNOWN')")
                val cursorP = db.query("PRAGMA table_info(pending_status_updates)")
                val colsP = mutableSetOf<String>(); while(cursorP.moveToNext()) colsP.add(cursorP.getString(1)); cursorP.close()
                val selectP = "id, lat, lng, speed, accuracy, bearing, battery, temp, isCharging, " +
                             (if(colsP.contains("currentMa")) "currentMa" else "0") + ", timestamp, " +
                             (if(colsP.contains("gpsTs")) "gpsTs" else "0") + ", satsView, satsUsed, " +
                             (if(colsP.contains("name")) "name" else "NULL") + ", maxAccuracy, distToTracker, distToHome, " +
                             (if(colsP.contains("snrIdx")) "snrIdx" else "0.0") + ", " +
                             (if(colsP.contains("isBatterySteepDischarge")) "isBatterySteepDischarge" else "0") + ", " +
                             (if(colsP.contains("isCoolingModeActive")) "isCoolingModeActive" else "0") + ", " +
                             (if(colsP.contains("isSitDetected")) "isSitDetected" else "0") + ", " +
                ...
                db.execSQL("DROP TABLE pending_status_updates")
                db.execSQL("ALTER TABLE pending_status_updates_v36 RENAME TO pending_status_updates")
                db.execSQL("CREATE INDEX IF NOT EXISTS index_pending_status_updates_timestamp ON pending_status_updates (timestamp)")
            }
        }
