...
files_gps = [
    ...
    "STATUS/requirements_sot_archive.md"
]
...
files_234 = [
    ...
    "234/STATUS/requirements_sot_archive.md"
]
...
print("--- GPS-Tracker (Current) ---")
for f in files_gps:
    res = count_in_file(f)
    if res:
        print(f"{f:35} | Issues: {res[0]:3} | Reqs: {res[1]:3}")

print("\n--- 234 (Historical Snapshot) ---")
for f in files_234:
    res = count_in_file(f)
    if res:
        print(f"{f:35} | Issues: {res[0]:3} | Reqs: {res[1]:3}")
